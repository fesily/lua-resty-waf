...
function _M.load_ruleset_file(name, version)
    for k in string_gmatch(package.path, "[^;]+") do
        ...
        if f ~= nil then
            ...
            local ruleset
            ruleset, err = _M.parse_ruleset(data)
            if err then
                return nil, err
            end
            return setmetatable(ruleset, { __index = { __version = time } })
        end
    end
